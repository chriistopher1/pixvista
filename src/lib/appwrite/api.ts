...
export async function createUserAuth(user: INewUser) {
  try {
...
export async function getCurrentUser() {
  try {
    ...
    const currentUserInfo = await databases.listDocuments(
      appwriteConfig.databaseId,
      appwriteConfig.userCollectionId,
      [Query.equal("accountId", import.meta.env.VITE_APPWRITE_USER_DEMO_ID)]
    );

    if (!currentUserInfo) throw Error;

    return currentUserInfo.documents[0];
  } catch (error) {
    console.log(error);
    return null;
  }
...
export async function isEmailAvailable(email: string) {
  try {
    ...
    if (isAvailable.documents[0]) return false;
    return true;
  } catch (error) {
    console.log(error);
  }
...
export async function createPost(post: INewPost) {
  try {
    ...
    const creatorDocumentId = user?.$id;
...
// ============================== SAVE POST
export async function savePost(userId: string | undefined, postId: string) {
  try {
